...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  const mySumAb = sum(a, b)[0];
  const mySumAbc = sum(mySumAb, c)[0];
  const myMultiplyAb = multiply(a, b)[0];
  const myMultiplyAbc = multiply(myMultiplyAb, c)[0];

  const mySumStr = `${a} and ${b} and ${c} sum to ${mySumAbc}.`;
  const myMultiplyStr = `The product of ${a} and ${b} and ${c} is ${myMultiplyAbc}.`;

  return [mySumAbc, myMultiplyAbc, mySumStr, myMultiplyStr];
}
...
function multiplyArray(multArr) { //eslint-disable-line
  if (!multArr || !Array.isArray(multArr) || multArr.length === 0) {
    // Handle the case where multArr is undefined, not an array, or an empty array.
    return [0, 'No valid input provided.'];
  }
  ...
  const product = multArr.reduce((accumulator, currentValue) => {
    if (typeof accumulator === 'number' && typeof currentValue === 'number') {
      ...
      // Handle cases where elements in the array are not numbers.
      return accumulator;
    }
  }, 1);
